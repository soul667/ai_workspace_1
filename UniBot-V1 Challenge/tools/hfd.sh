#!/usr/bin/env bash
#
# hfd - Hugging Face parallel downloader (aria2c/wget)
#
# Version: 2.0.0
# Author: luobaigu
# License: MIT
#
# Changes:
#   2.0.0  2026-07-22  Add HF cache structure post-processing (refs + snapshot symlinks)
#                      so vllm serve org/model works directly after download.
#   1.0.0             Initial release: parallel download via aria2c with resume support.
#
...
Options:
  ...
  --local-dir     (Optional) Directory path to store the downloaded data.
                             Defaults to \$HF_HOME/hub/models--org_name--repo_name
...
LOCAL_DIR="${LOCAL_DIR:-${HF_HOME:-$HOME/.cache/huggingface}/hub/models--${REPO_ID//\//--}}"
...
finalize_filelist() {
    ...
    : > "$mf"
    awk -F'\t' -v mf="$mf" '!seen[$2]++ { print >> mf; n++; s+=$1 } END { printf "%.0f %.0f", n, s }' \
...
build_download_list() {
    ...
    while IFS=$'\t' read -r size path; do
        ...
        if [[ "$TOOL" == "aria2c" ]]; then
            ...
            cur=$(stat -c%s "$path" 2>/dev/null || echo 0); (( cur > size )) && rm -rf "$path"
...
monitor_progress() {
    ...
    if [[ "$TOOL" == "wget" ]]; then
        ...
        # Parallel downloads finish out of order, so a cursor would miss files completing
        # between ticks. One metadata walk per tick stays correct: block usage is sparse-aware,
        # completed = data files minus in-progress (.aria2 sidecar) ones. Reads cached metadata.
        local now files t0 walk
        while :; do
            sleep "$interval"
            t0=$SECONDS
            read -r now files < <(find . -type f ! -path './.hfd/*' -printf '%b %p\n' 2>/dev/null \
                | awk '/\.aria2$/{a++;next} {b+=$1;d++} END{printf "%d %d", b*512, d-a}')
...
# Normalize scientific notation (e.g. 1.68068e+10) to integer for bash arithmetic
[[ "$TOTAL_SIZE" =~ [eE] ]] && TOTAL_SIZE=$(printf '%.0f' "$TOTAL_SIZE")
...
if [[ $status -eq 0 ]]; then
    ...
    # ---- Create HF cache structure (refs + snapshot symlinks) ----
    printf "%bCreating HF cache structure...%b" "$DIM" "$NC"
    SHA=$(curl -s -o /dev/null -w "%{redirect_url}" \
        ${HF_TOKEN:+-H "Authorization: Bearer $HF_TOKEN"} \
        "$HF_ENDPOINT/$DOWNLOAD_API_PATH/resolve/$REVISION" 2>/dev/null | \
        grep -oE '(/commit/|/tree/)[a-f0-9]{40}' | grep -oE '[a-f0-9]{40}' | head -1)
    if [[ -z "$SHA" ]]; then
        SHA=$(curl -s ${HF_TOKEN:+-H "Authorization: Bearer $HF_TOKEN"} \
            "$HF_ENDPOINT/api/$REPO_API_PATH/revision/$REVISION" 2>/dev/null | \
            grep -o '"sha":"[a-f0-9]*"' | cut -d'"' -f4)
    fi
    if [[ -n "$SHA" ]]; then
        mkdir -p "refs" "snapshots/$SHA"
        printf '%s' "$SHA" > "refs/main"
        find . -type f ! -path './.hfd/*' ! -path './refs/*' ! -path './snapshots/*' ! -path './blobs/*' -print0 |
        while IFS= read -r -d '' SRC; do
            REL="${SRC#./}"
            DST="snapshots/$SHA/$REL"
            mkdir -p "$(dirname "$DST")"
            TARGET="$(realpath --relative-to="$(dirname "$DST")" "$SRC")"
            ln -sfn "$TARGET" "$DST"
        done
        printf '\r\033[K%bCache structure created: %s/refs/main → %s%b\n' "$GREEN" "$LOCAL_DIR" "$SHA" "$NC"
    else
        printf '\r\033[K%bWarning: could not get commit SHA. Cache structure not created.%b\n' "$YELLOW" "$NC"
    fi
else
